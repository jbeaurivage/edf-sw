...
use edf_sw_scheduler::benchmark::reset_cyccnt;
use edf_sw_scheduler::fugit::{Duration, ExtU32};
...
fn main() -> ! {
    ...
    SCHEDULER.schedule(Task::new(Deadline::millis(500).ticks(), 0, software_task));
...
fn SysTick() {
    SCHEDULER.schedule(Task::new(Deadline::millis(2).ticks(), 2, systick_task));
}
...
fn TC4() {
    ...
    SCHEDULER.schedule(Task::new(Deadline::millis(20).ticks(), 1, timer_task));
}
